...
const LandingHeader = ({ shop, isAuthenticated, onShopAuthChange, cart, shopUser }) => {
    ...
    <div style={shop.styles.nav} className="landing-header-wrapper">
      <div style={shop.styles.nav} className="container landing-navbar2">
        ...
        <div  className="navbar-items2">
            ...
            {(isAuthenticated () && shopUser.name === shop.name) ? 
            ...
            {(isAuthenticated() && shopUser.name === shop.name) && <div className="nav-bar-li3 newli4"><Link style={shop.styles.navLinks} className="Link" to={`/shops/${shop.urlName}`}><i className="fa fa-sign-out" onClick={e => {
              e.preventDefault()
